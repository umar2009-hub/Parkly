...
export const OwnerLayout: React.FC = () => {
  ...
  const navItems = [
    { name: 'Overview', path: '/owner', icon: LayoutDashboard, exact: true },
    { name: 'My Parking', path: '/owner/parking', icon: Building },
    { name: 'Live Operations', path: '/owner/operations', icon: Activity },
    { name: 'Access Logs', path: '/owner/access-logs', icon: FileSpreadsheet },
    { name: 'Invoices', path: '/owner/invoices', icon: Receipt },
    { name: 'Slots Status', path: '/owner/slots', icon: Key },
    { name: 'Revenue', path: '/owner/revenue', icon: Landmark },
    ...
    <div className="min-h-screen bg-brand-charcoal text-white font-sans flex flex-col md:flex-row relative">
      ...
      <aside className="hidden md:flex md:w-64 bg-brand-surface border-r border-brand-surface-hover flex-col shrink-0">
        <div className="p-6 border-b border-brand-surface-hover flex items-center space-x-2">
          ...
          <span className="text-[9px] bg-brand-lime/10 text-brand-lime border border-brand-lime/20 px-1 rounded font-mono uppercase tracking-wider">Owner</span>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-brand-charcoal min-h-screen pb-16 md:pb-0">
        ...
        <header className="hidden md:flex bg-brand-surface/40 border-b border-brand-surface-hover px-8 py-4 justify-between items-center">
          <h2 className="text-sm font-mono text-brand-text-muted uppercase tracking-wider">
            {location.pathname.split('/').slice(2).join(' / ') || 'Overview'}
          </h2>
      ...
      {/* ==========================================
          MOBILE BOTTOM NAVIGATION (Owner version)
         ========================================== */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-brand-surface border-t border-brand-surface-hover grid grid-cols-5 py-2">
        {[
          { name: 'Home', path: '/owner', icon: LayoutDashboard, exact: true },
          { name: 'Parking', path: '/owner/parking', icon: Building },
          { name: 'Slots', path: '/owner/slots', icon: Key },
